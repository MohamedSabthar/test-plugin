            ...
            doLast {
                ...
                if (needBuildWithTest) {
                    ...
                    project.exec {
                        ...
                        if (checkForBreakingChanges) {
                             commandLine 'sh', '-c', "docker run --net=host  --user \$(id -u):\$(id -g) -v $project.projectDir:/home/ballerina -v $project.projectDir/..:/home ballerina/ballerina:nightly $balJavaDebugParam bal pack --target-dir ${balBuildTarget} --offline ${debugParams}"
                    ...
                    if (!skipTests) {
                        project.exec {
                            ...
                            environment 'JAVA_OPTS', '-DBALLERINA_DEV_COMPILE_BALLERINA_ORG=true'
                            if (checkForBreakingChanges) {
                                commandLine 'sh', '-c', "docker run --net=host  --user \$(id -u):\$(id -g) -v $project.projectDir:/home/ballerina -v $project.projectDir/..:/home ballerina/ballerina:nightly $balJavaDebugParam bal test --offline ${testCoverageParams} ${groupParams} ${disableGroups} ${debugParams}"
                            } else if (Os.isFamily(Os.FAMILY_WINDOWS)) {
            ...
            doLast {
                if (needSeparateTest) {
                    ...
                    project.exec {
                        ...
                        environment 'JAVA_OPTS', '-DBALLERINA_DEV_COMPILE_BALLERINA_ORG=true'
                        if (checkForBreakingChanges) {
                             commandLine 'sh', '-c', "docker run --net=host --user \$(id -u):\$(id -g) -v $project.projectDir:/home/ballerina -v $project.projectDir/..:/home ballerina/ballerina:nightly bal test --offline ${testCoverageParams} ${groupParams} ${disableGroups} ${debugParams}"
                        } else if (Os.isFamily(Os.FAMILY_WINDOWS)) {
                            commandLine 'cmd', '/c', "${balJavaDebugParam} ${distributionBinPath}/bal.bat test --offline ${testCoverageParams} ${groupParams} ${disableGroups} ${debugParams} && exit %%ERRORLEVEL%%"
                        } else {
                            commandLine 'sh', '-c', "${balJavaDebugParam} ${distributionBinPath}/bal test --offline ${testCoverageParams} ${groupParams} ${disableGroups} ${debugParams}"
